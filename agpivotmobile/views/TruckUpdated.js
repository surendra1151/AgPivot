...
export default class TruckUpdated extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      field: params.field,
      comb: params.comb,
      prev_length: params.prev_length,
      truck: '',
      isDisplayLogout: false,
      selected_truck: '',
      post_data_truckUpdated: '',
      length: 0,
      net_info: false,
      day_summary: '',
      width: width,
  ...
  componentDidMount(): void {
    console.log('The length getting from prev screen:', this.state.prev_length);
    ...
    AsyncStorage.getItem('loginCredentials').then(value => {
      ...
      AsyncStorage.getItem('harvestInfo').then(value => {
        ...
        AsyncStorage.getItem('postDataInfo').then(value => {
          console.log(
            'In Post Data Info is in TruckUpdated Screen is:',
            JSON.parse(value),
          );
          this.setState({
            post_data_truckUpdated: JSON.parse(value),
          });
  ...
  handleBackButtonClick() {
    ...
    if (this.state.length === 0) {
      ...
      // this.props.navigation.replace('FieldVariety', {name: this.state.operator_name,
      //     operator_name: (this.state.harvester_name)});
      // this.props.navigation.pop(2);
      this.props.navigation.goBack();
    } else {
      // alert('There are un-synced records, field change is possible only on syncing the records!')
      alert(
        'There are un-synced records. Please, sync these records before changing Field-Variety',
      );
    }
  ...
  checkNetInfo(name) {
    console.log(
      'I am in Checking Net Info in Truck Screen values are:',
      this.state.length,
      this.state.post_data_truckUpdated,
    );
    NetInfo.fetch().then(state => {
      this.setState({
        net_info: state.isConnected,
      ...
      if (state.isConnected) {
        if (this.state.length !== 0) {
          fetch(baseUrl + '/harvest', {
            ...
            body: JSON.stringify(this.state.post_data_truckUpdated),
          })
            ...
            .then(responseJson => {
              if (responseJson.error !== true) {
                ...
                AsyncStorage.setItem(
                  'lastHarvestedAcres',
                  JSON.stringify(responseJson.daySummary.accum_list),
                );
                AsyncStorage.removeItem('postDataInfo').then(value => {
                  console.log(
                    'The data is Uploaded & deleted in Truck Updated Screen is:',
                    responseJson,
                  );
                  this.setState({
                    length: 0,
          ...
          // alert("There is no Data to get synced in TruckUpdated Screen:");
        }
      } else {
        // if(name === "onPress") {
    ...
    for (var key in this.state.truck) {
      if (this.state.truck[key] === item) {
        ...
        this.props.navigation.replace('Data', {
          harvester_name: this.state.harvester_name,
          operator_name: this.state.operator_name,
          field: this.state.field,
          comb: this.state.comb,
          truck: key_in_object,
      ...
      <View>
        <View
          ...
          <TouchableOpacity onPress={() => this.displayLogout()}>
            <Image
              source={require('../assets/Alsum.png')}
        ...
        {this.state.isDisplayLogout ? (
          <View
            style={{
              ...
              alignSelf: 'center',
              marginLeft: 0.5 * this.state.width,
              zIndex: 100000,
            }}>
            <Modal
              isVisible={this.state.isDisplayLogout}
              onBackdropPress={() => this.setState({isDisplayLogout: false})}
              style={{
                marginBottom: this.state.height / 3,
                marginTop: this.state.height / 5,
                marginLeft: this.state.width / 2 - 160,
              ...
              <View style={{alignItems: 'center'}}>
                ...
                <TouchableOpacity
                  onPress={() => this.about()}
                  style={{marginBottom: 15, zIndex: 1, marginTop: 50}}>
  ...
  logout() {
    if (this.state.length !== 0) {
      alert(
        'There are un-synced records, logout is possible only after syncing all records!',
      );
    } else {
      ...
      fetch(baseUrl + '/harvest-data', {
        ...
        .then(responseJson => {
          if (responseJson.error !== true) {
            ...
            alert(
              "The field info can't be synced right now, Please try after some time!",
            );
            this.setState({
              isError: true,
              isLogin: false,
  ...
  render() {
    ...
    console.log(
      'Just after Syncing Data the values are in TruckUpdated Screen is:',
      this.state.post_data_truckUpdated,
      this.state.length,
      this.state.prev_length,
    );
    return (
      <View style={{flex: 1}}>
        <View
          ...
          <TouchableOpacity onPress={() => this.goToHomeScreen()}>
            <View style={{marginLeft: 30, top: 6}}>
              <Image
                ...
                style={{height: 28, width: 80.62, marginTop: 5}}
              />
            </View>
          </TouchableOpacity>
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            <View>{this.printImage()}</View>
            <TouchableOpacity
              onPress={() => this.checkNetInfo('onPress')}
              style={{flexDirection: 'row', right: 25}}>
              <View
                style={{
                  borderRadius: 50,
                  width: 20,
              ...
              <Image
                ...
                style={{height: 30, width: 30, top: 10, right: 10, zIndex: -1}}
              />
            </TouchableOpacity>
            {/*{this.state.isDisplayLogout ?*/}
            {/*    <View style={{*/}
                    ...
                    <View
                      ...
                      <View style={{width: 300}}>
                        ...
                        <Text style={styles.trucks_table_text}>
                          Field Truck:{' '}
                          {this.state.local_summary.daySummary.day_count}
                        </Text>
                        <Text style={styles.trucks_table_text}>
                          Acres Harvested:{' '}
                          {this.state.local_summary.daySummary.day_acres}
                        </Text>
                      </View>
                    ...
                    <View
                      ...
                      <View style={{flexDirection: 'row'}}>
                        <View style={{flex: 1, alignItems: 'center'}}>
                          <Text style={[styles.trucks_table_text]}>Truck</Text>
                        </View>
                        <View style={{flex: 1, alignItems: 'center'}}>
                          <Text style={[styles.trucks_table_text]}>
                            Acres Accumulated
                      ...
                      />
                      {this.state.local_summary !== '' &&
                      this.state.local_summary !== null ? (
                        <View>
                          {this.state.local_summary.daySummary.last_ten.length >
                          0 ? (
                            <FlatList
                              data={
                                this.state.local_summary.daySummary.last_ten
                              }
                        ...
                        <FlatList
                          ...
                          renderItem={({item}) => (
                            <LinearGradient
                              ...
                              <TouchableOpacity
                                onPress={() => this.gotoDataEntryScreen(item)}>
                                <View
                                  style={{
